...
var CustomImportScript = (() => {
  ...
  // tools/importer/import-find-doctor.js
  var import_find_doctor_exports = {};
  __export(import_find_doctor_exports, {
    default: () => import_find_doctor_default
  });
  ...
  // tools/importer/parsers/find-a-doctor.js
  function parse2(element, { document: document2 }) {
    ...
    rows.forEach((row) => {
      ...
      const keyEl = inner[0];
      const valEl = inner[1];
      const keyContent = keyEl.children.length ? Array.from(keyEl.children) : keyEl.textContent.trim();
      const valContent = valEl.children.length ? Array.from(valEl.children) : valEl.textContent.trim();
    ...
    const block = WebImporter.Blocks.createBlock(document2, { name: "find-a-doctor", cells });
    element.replaceWith(block);
  }
  ...
  var import_find_doctor_default = {
    transform: (payload) => {
  ...
  return __toCommonJS(import_find_doctor_exports);
})();
